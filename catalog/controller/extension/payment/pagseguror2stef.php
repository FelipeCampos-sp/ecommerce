...
class ControllerExtensionPaymentPagSeguroR2STef extends Controller {
	...
	public function index() {
		...
		$data['gateway'] = $this->url->link('extension/payment/pagseguror2stef/gateway','','SSL');
		//cliente
		...
		if(version_compare(VERSION, '2.1.9.9', '<=')){
			return $this->load->view('default/template/extension/payment/pagseguror2stef.tpl', $data);
		}else{
			return $this->load->view('extension/payment/pagseguror2stef', $data);
		}
	}
	...
	public function gateway(){
		...
		//pega o fiscal
		$fiscal = preg_replace('/\D/', '', $_POST['cpf_tef']);
		
		//dados telefone
		$dddt = $telt = '';
		$tell_fullt = substr(preg_replace('/\D/', '',$order_info['telephone']),-11);
		...
		$xmlps['paymentMethod'] = 'eft';
		$xmlps['receiverEmail'] = trim($email_pagseguro);
		$xmlps['reference'] = $order_info['order_id'];
		$xmlps['senderName'] = utf8_decode(trim(preg_replace('!\s+!',' ',$order_info['firstname'].' '.$order_info['lastname'])));
		$xmlps['senderEmail'] = $order_info['email'];
		$xmlps['senderAreaCode'] = $dddt;
		$xmlps['senderPhone'] = $telt;
		if(strlen($fiscal)==11){
		$xmlps['senderCPF'] = $fiscal;	
		}elseif(strlen($fiscal)==14){
		$xmlps['senderCNPJ'] = $fiscal;	
		}
		$xmlps['notificationURL'] = $this->url->link('extension/payment/pagseguror2stef/ipn','','SSL');
		...
		$xmlps['itemDescription'.$i] = utf8_decode($produto['name']);
		...
		//hash do cliente
		$xmlps['senderHash'] = trim($_POST['hash_tef']);
		$xmlps['bankName'] = trim($_POST['banco_tef']);
		...
		$log .= '<a href="'.$dados_pagseguro['paymentLink'].'" class="btn btn-success btn-xs" target="_blank"><i class="fa fa-shopping-cart"></i> link de pagamento</a>';
		}
		$this->model_checkout_order->addOrderHistory($dados_pagseguro['reference'], $status_id['id'],$log,true);
		$data['cupom'] = $this->url->link('extension/payment/pagseguror2stef/cupom&id='.$dados_pagseguro['code'].'&venda='.$this->session->data['order_id'].'','','SSL');
		...
		if($data['erro']){
			//se erro
			$this->log->write("Erro Pagseguro TEF - ".$this->tratar_mensagem_erro($data['msg']));
