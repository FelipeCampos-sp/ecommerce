...
class ControllerExtensionPaymentPagSeguroR2SBoleto extends Controller {
	...
	public function index() {
		...
		$data['gateway'] = $this->url->link('extension/payment/pagseguror2sboleto/gateway','','SSL');
		//cliente
		...
		if(version_compare(VERSION, '2.1.9.9', '<=')){
			return $this->load->view('default/template/extension/payment/pagseguror2sboleto.tpl', $data);
		}else{
			return $this->load->view('extension/payment/pagseguror2sboleto', $data);
		}
	}
	...
	public function gateway(){
		...
		//pega o fiscal
		$fiscal = preg_replace('/\D/', '', $_POST['cpf_boleto']);
		...
		$xmlps['notificationURL'] = $this->url->link('extension/payment/pagseguror2sboleto/ipn','','SSL');
		...
		//hash do cliente
		$xmlps['senderHash'] = trim($_POST['hash_boleto']);
		...
		$data['cupom'] = $this->url->link('extension/payment/pagseguror2sboleto/cupom&id='.$dados_pagseguro['code'].'&venda='.$this->session->data['order_id'].'','','SSL');
		...
		if($data['erro']){
			//se erro
			$this->log->write("Erro Pagseguro Boleto - ".$this->tratar_mensagem_erro($data['msg']));
